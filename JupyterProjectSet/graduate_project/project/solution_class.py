# -*- coding: utf-8 -*-
# @CreateTime : 2023/2/12 21:50
# @Author : 20866
# @IDE : PyCharm
# @File : PycharmProject/solution_class.py
# @Description : 爬慕课评论
# @Interpreter : python 3.10
# @Motto : You must take your place in the circle of life!
# @Site : https://github.com/JinYu-2023 or 
    ...
    def get_course_evaluation(self) -> list[any]:
        ...
        while True:
            try:
                ...
                print('CanNotFoundElement!')
            else:
                time.sleep(1)
                for item in one_page_comment_item:
                    comment_text = item.text
                    comment_list_one_page.append(comment_text)
            time.sleep(0.5)
            if GetText.click_evaluation_next(self) == 0:
                continue
            else:
                return comment_list_one_page
        return comment_list_one_page
...
if __name__ == '__main__':
    ...
    x = GetText(driver)  # 初始化GetText类

    GetText.open_url(x, 'https://www.icourse163.org/')
    GetText.search_key(x, 'python')
    element = driver.find_elements(By.CLASS_NAME, 'f-fcorange.f-ib')
    element[0].click()
    GetText.second_handle(x)
    print(GetText.get_course_evaluation(x))
